...
- 삭제 진입점은 휴관일 관리 카드 케밥 `삭제` → 삭제 확인 다이얼로그다. 수정 화면에는 삭제 버튼이 없다.
- 확인 중에는 다이얼로그가 pending 상태이고 `deletingRef`로 중복 요청을 막는다.
- 성공 시 `['close-schedules']`를 무효화해 카드와 캘린더 마커가 사라지고 성공 토스트 `데이터 삭제에 성공했습니다`를 표시한다.
- 실패(오류 status, Contract 밖 성공 status·body 포함) 시 다이얼로그를 닫고 목록을 유지하며 오류 토스트 `데이터 삭제에 실패했습니다`를 표시한다.
...
- API Contract는 바뀌지 않았다. 삭제 진입이 수정 화면에서 카드 케밥으로 옮겨져 시나리오를 갱신한다. 수정 화면 직접 진입 삭제(S9)는 목록 새로고침 후 케밥 삭제로 대체한다.
