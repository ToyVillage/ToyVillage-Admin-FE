...
- 사이드바 `개체관리 > 개체 카드` 클릭 → `/species` 로 이동하고 사이드바가 닫힌다. `/species/**` 하위 경로(종 상세·폼, 개체·관찰 화면)에서도
  이 하위 항목이 선택 상태다(`sidebar.spec.md` 메뉴 절, (2026-09-15 개발자 결정)).
...
- S29: 사이드바 `개체관리 > 개체 카드` 클릭 → `/species` 로 이동하고 사이드바가 닫힌다. 종 상세(`/species/1`)에서 사이드바를 열면 이 하위 항목이 선택 상태다.
...
- 사이드바 `개체관리 > 개체 카드` 에 `/species` 를 연결하고 `/species/**` 하위 경로도 선택 상태로 본다 (2026-09-15 개발자 결정).
  처음에는 평면 메뉴 `개체관리 바로가기` 를 추가했으나, develop 병합으로 사이드바가 아코디언이 되어 기존 하위 항목으로 옮겼다.
