...
- 2026-09-17: 생성 성공·실패 피드백을 yot 기준으로 반영했다(#133). 성공 → 목록 토스트 `1:6158`, 실패 → 생성 화면 모달 `1:7020`. 목록 실패 토스트 `1:6180`은 실패 시 목록으로 이동하지 않으므로 쓰지 않는다.
...
- 성공 → 휴관 일정 목록 query를 갱신하고 `/notices/guide`로 이동한다. 목록은 `데이터 생성에 성공했습니다` 성공 토스트를 보인다(Figma `1:6158`). 토스트를 닫으면 이동 state를 비워 새로고침 때 다시 뜨지 않는다.
- 실패 → `데이터 생성에 실패했습니다` 모달과 `확인` 버튼을 보인다(Figma `1:7020`, 공용 `ErrorDialog`). `확인` 또는 Escape로 닫으면 현재 입력값과 페이지를 유지하고 재시도할 수 있다.
...
- S7: 유효한 시작일, 종료일과 제목으로 생성 성공 → 요청이 한 번 전송되고 `/notices/guide`로 이동하며 `데이터 생성에 성공했습니다` 토스트가 보인다.
- S8: 생성 실패 → `데이터 생성에 실패했습니다` 모달이 보이고, `확인` 후 입력값과 현재 페이지가 유지되어 다시 생성할 수 있다.
