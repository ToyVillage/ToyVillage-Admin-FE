...
- `src/features/species-form/ui/SpeciesForm.tsx` (저장 전 없는 이름 생성)
...
- 법정분류 추가 모달에서 `추가하기` 시 요청 body `{ kind: <앞뒤 공백 제거한
  이름> }`으로 호출한다.
  - 기존 중복 검사(서버 목록·저장값 이름과 같으면 `이미 있는 분류입니다!`)는
    호출 전에 그대로 한다.
...
- 추가한 항목은 공용 목록이라 다른 종 폼에도 선택지로 보인다(2026-09-16
  개발자 결정).
- 종 저장 시 선택된 이름 중 서버 목록에 없는 항목은 이 API로 먼저 만들고
...
- 종 저장 전 이 생성이 실패하면 종 생성·수정을 호출하지 않고 기존 폼
  저장 실패 표시를 쓴다.
