...
- **yot 실측 확인**: 표 골격·컬럼 고정폭·셀 padding·담당자 2텍스트·케밥 크기(`145:11450`),
  ...
  헤더 규격(`1:3270`), 탭바 규격(`1:3271`), 그리고 스크린샷 픽셀 샘플링으로 확인한 색 전체.
...
- `전체 업무` 탭 클릭 → 모든 상태의 업무가 다시 보인다. `반려` 는 전용 탭이 없고 `전체 업무` 에서만 보인다.
...
4. 상태 pill(`status / 업무 지시`): 80×40, radius 80px, 20px Medium.
   ...
   - `반려` — 배경 `colors.warningBg`(`#FFE8C3`), 글자 `colors.warning`(`#FDB542`)
...
- 상태(`진행중/완료/반려`)는 표시 전용 파생값이며 목록에서 변경 UI 를 만들지 않는다.
- 한 페이지 4행을 유지한다(Figma 표 높이 472 = 헤더 72 + 행 100 × 4).
