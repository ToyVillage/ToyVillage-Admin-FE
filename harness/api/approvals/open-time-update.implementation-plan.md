# Implementation Plan — open-time-update

## 승인 기준

- `PUT /open-time/{open-time-id}`, body `{ openDate, startOpenTime, endOpenTime }`, 시간 `HH:mm`, 성공 HTTP 201 + `message` string.
- 조회 응답 `id`가 있는 날짜에서만 호출하고 path에 그 id를 쓴다.
...
- `src/entities/operating-hours/api/types.ts` — `OpenTimeUpdateRequest`, `OpenTimeUpdateResponse`
- `src/entities/operating-hours/api/operatingHoursApi.ts` — `updateOperatingHours`, 조회 결과에 `id` 포함
...
- `tests/e2e/api/open-time-update.spec.ts`
...
1. `yarn harness:api:validate open-time-update`
2. `yarn harness:api:gate open-time-update`
3. `yarn harness:api:policy open-time-update <변경 파일>`
4. `yarn verify`
5. `yarn verify:api open-time-update`
