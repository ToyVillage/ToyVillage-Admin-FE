...
- 선택지 표시 순서(2026-09-18 개발자 결정, 이슈 #149 — 기본 선택지 폐기):
  1. 서버 목록을 응답 순서대로 ✕ 있는 pill로 보인다. 화면이 고정으로 들고 있는
     기본 선택지는 없다.
  2. 그 뒤에 저장값 중 목록에 없는 이름(다른 곳에서 삭제된 항목)을 선택 상태로 붙인다.
  3. 마지막에 `+ 법정분류 추가`.
...
- 종 저장 시 선택된 이름을 이 목록에서 찾아 `animalLegalDesignation` id
  배열로 바꾼다. 선택된 이름이 서버 목록에 없으면(다른 곳에서 삭제) 먼저
  `animal-legal-status-create`로 만든 뒤 목록을 다시 받아 id를 찾는다.
