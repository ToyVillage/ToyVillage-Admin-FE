...
- **Figma 근거 없음(게이트 ② 결정으로 채움)**: 관찰 0건 빈 상태 문구, 행 케밥 메뉴 위치(이 feature 에 행 케밥 열림 프레임 없음),
  ...
  `먹이 급여 기록 확인하기` 이동 대상·이력 없음/조회 실패 토스트, 없는 id 처리, 접근성·반응형 절 전체.
...
- 클릭·Enter → 이 개체의 급여 이력(`GET /feed-log/admin/history/{animalManageId}`)을 조회한다.
  - 이력이 있으면 → 가장 최근 급여 기록 상세 `/feeds/:feedLogId` 로 이동한다.
    급여 상세는 그 기록과 이 개체의 급여 이력 전체를 보여준다. 급여 상세의 `뒤로가기` 는 이 개체 상세로 돌아온다.
  - 이력이 없으면(404 포함) → 이동하지 않고 `급여 기록이 없습니다` 오류 토스트를 띄운다.
  - 조회에 실패하면(500·네트워크) → 이동하지 않고 `급여 기록을 불러오지 못했습니다` 오류 토스트를 띄운다.
- 버튼은 항상 누를 수 있고 시각은 Figma 외형 그대로 둔다.
...
- `먹이 급여 기록 확인하기` 는 버튼이다. 이동하지 못한 이유는 토스트(`role="alert"`)로 알린다.
...
- S31: 급여 이력이 없는 개체에서 `먹이 급여 기록 확인하기` 클릭 → 경로가 바뀌지 않고 `급여 기록이 없습니다` 토스트가 뜬다.
- S32: 급여 이력 조회 실패 시 `먹이 급여 기록 확인하기` 클릭 → 경로가 바뀌지 않고 `급여 기록을 불러오지 못했습니다` 토스트가 뜬다.
...
- `먹이 급여 기록 확인하기` 는 이 개체의 가장 최근 급여 기록 상세(`/feeds/:feedLogId`)로 이동한다. 개체 기준 급여 이력 화면을 따로 두지 않는다 — 급여 상세가 이미 그 개체의 급여 이력 전체를 보여준다. 이력은 누를 때 조회하고, 없거나 불러오지 못하면 오류 토스트로 알린다(토스트 규격은 삭제 결과 토스트 승계) (2026-09-17 개발자 결정, #119: 개체별 급여 이력 Figma 프레임 없음).
