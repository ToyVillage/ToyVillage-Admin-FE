...
- S5. `401 { message: "만료된 토큰입니다.", status:401, ... }` → 재발급 시도 없이 저장된 토큰을 비우고 `/login` 으로 이동(생성 성공 처리 없음). 전제: 세션 fixture 에 refresh token 이 없고 APP_AUTH_REISSUE mock 도 없다. 재발급 성공 경로는 app-auth-reissue 시나리오가 담당한다. 전역 인증 처리 도입으로 실패 다이얼로그에서 개정.
