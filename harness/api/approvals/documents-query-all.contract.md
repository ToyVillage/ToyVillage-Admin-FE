...
- 응답은 래퍼 없는 배열(총 개수/전체 페이지 메타 없음).
- 페이지 경계 판단: 총개수가 없으므로 반환 배열 길이로 다음 페이지 유무를 추정한다(길이 = size(10)면 다음 있음으로 간주). 마지막 페이지가 정확히 10개면 빈 다음 페이지가 노출될 수 있어, 빈 배열 응답을 받으면(1페이지 제외) 직전 페이지로 자동 복귀해 고착을 막는다. 정식 번호 페이지네이션은 백엔드가 총개수(예: `totalPages`/`X-Total-Count`)를 제공할 때 전환한다.
