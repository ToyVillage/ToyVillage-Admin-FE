...
- 제외: **승인·반려 결과 표시(성공 토스트·실패 토스트·실패 모달) — 개발자 결정으로 다음 슬라이스**,
  ...
  재제출 요청 처리, 검색·정렬·다중 선택, 사이드바 메뉴 변경(Figma 사이드바에 업무보고 항목이 없다 — TODO-2)
...
- 목록(`/task-reports`)은 이번 범위에서 화면 안에 진입 버튼을 두지 않는다(Figma 근거 없음 — TODO-2).
...
- **TODO-2 (목록 진입 경로):** 업무 상세의 `업무 보고 상세조회` 는 **상세 단건**으로 가는 버튼이라, 목록 화면으로 가는
  진입점은 아직 없다(Figma 사이드바 `1541:1423`~ 에도 업무보고 항목이 없다). 이번 범위에서는 사이드바를 바꾸지 않고
  목록은 URL 진입으로만 접근한다.
