...
- S4. 500 → 저장 실패 다이얼로그.
- S6. 401 만료된 토큰 → 재발급 시도 없이 저장된 토큰을 비우고 `/login` 으로 이동(저장 성공 처리 없음). 전제: 세션 fixture 에 refresh token 이 없고 APP_AUTH_REISSUE mock 도 없다. 재발급 성공 경로는 app-auth-reissue 시나리오가 담당한다. 전역 인증 처리 도입으로 실패 다이얼로그에서 개정.
