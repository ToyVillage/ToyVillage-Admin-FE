...
- 헤더 셀(질문명)은 항상 한 줄이고 잘리지 않는다. 열 폭을 질문명 폭 이상(최소 200px,
  장문형 320px)으로 잡고, 본문 셀은 그 폭을 그대로 따른다.
- 각 질문 셀은 질문 유형에 따라 다르게 표기한다(Figma `541:14081`).
  - 단답형 / 객관식 질문 / 드롭다운 → 값 텍스트 한 줄, 열 폭을 넘으면 말줄임한다.
  - 장문형 → 값 텍스트 한 줄, 열 폭을 넘으면 말줄임(`…`)한다.
  - 체크박스 → 선택된 값마다 chip 하나를 가로로 나열하되, 셀 폭에 들어가는 만큼만 보이고
    나머지는 `+N` 으로 접는다(chip 을 반쯤 자르지 않는다).
  - 파일 업로드 → 첨부 칩(유형 아이콘 + 파일명 + 다운로드, `AttachmentChip`). 첨부가 없으면 빈 셀.
    2026-09-17 에 빈 셀에서 첨부 칩으로 바꿨다(개발자 승인). 다운로드 실패는 토스트로 알린다.
- 값이 있는 셀은 클릭하면 질문명과 전체 값을 팝오버로 보여준다. 같은 셀을 다시 누르면 닫히고,
  `Esc` 와 바깥 클릭으로도 닫힌다. 스크롤·리사이즈에는 닫지 않고 기준 셀을 따라간다.
  hover 시에는 브라우저 기본 툴팁으로 전체 값을 보여준다.
  팝오버 크기는 내용이 정한다(고정 폭·높이 없음). 화면을 넘지 않도록 최대 폭 560px,
  화면 가장자리 여백 16px 만 둔다.
  실제 데이터에서 한 줄 말줄임만으로는 값을 읽을 수 없어 2026-09-17 에 추가했다(개발자 승인).
...
- `WorkLogSheet { columns, rows, onDownloadError }` (entities/work-log) — 시트 표. 질문 유형별 셀 렌더링을 담당한다.
  - `columns: { id, label, type }[]` — `type` 은 `SHORT_TEXT | LONG_TEXT | CHOICE | CHECKBOX | DROPDOWN | FILE`
  - `rows: { zone, values: Record<questionId, string | string[] | { fileName, fileKey } | null> }[]`
- `BackLink { to, children }` (shared/ui) — Figma `back`(1:10470). 예약 상세의 `ReservationBackLink` 와
  같은 규격이라 공용으로 올린다.

## 비고 / 제약

- 열 폭 규칙은 Figma `1:5694` 를 기준으로 삼는다: `설정된 구역` 160px 고정, 장문형 열 320px,
  그 밖의 질문 열 200px. 단 질문명이 그보다 길면 헤더가 잘리지 않도록 그 열만 넓어진다. `541:14081` 의 열 폭은 7개 열을 1320 안에 우겨넣은 레퍼런스용 값이라 따르지 않는다.
- 질문 수가 많아 1320px 을 넘으면 시트만 가로로 스크롤한다(페이지 본문은 가로 스크롤되지 않는다).
  열은 줄어들지 않는다. 축소를 허용하면 스크롤 대신 모든 열이 찌그러져 값이 서로 겹친다.
- 가로 스크롤 중에도 `설정된 구역` 열은 왼쪽에 고정(sticky)한다. 어느 구역의 행인지 놓치지 않기 위해
  2026-09-17 에 추가했다(개발자 승인). Figma `1:5694` 에는 없는 동작이다.
