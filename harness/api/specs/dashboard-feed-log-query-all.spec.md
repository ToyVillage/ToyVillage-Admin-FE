---
feature: dashboard-feed-log-query-all
api_id: DASHBOARD_FEED_LOG_QUERY_ALL
target_page: src/pages/dashboard/DashboardPage.tsx
notion_page: https://app.notion.com/p/aaf7a4d614748394bd4501dac6a97ea8
...
대시보드 `먹이 급여 관리` 카드에 이번 주 급여일지 최신 3건(`GET /dashboard/feed-logs`)을 표시한다.
...
- API ID: `DASHBOARD_FEED_LOG_QUERY_ALL`
...
2. 데이터베이스 엔드포인트 값에 Query String(`?page=1&size=10`)이 붙어 있다. Path는 `/dashboard/feed-logs`로 분리했다.
