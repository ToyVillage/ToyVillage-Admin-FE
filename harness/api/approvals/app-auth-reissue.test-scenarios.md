...
- 서로 다른 두 보호 API 가 동시에 401 → 재발급 요청 총 1회
- 두 요청 모두 새 access token 으로 재시도된다
...
## S5: 재발급 400 / 500 → S3 과 동일 (세션 정리 + /login)
...
## S10: 이미 /login 화면이면 다시 리다이렉트하지 않는다

- `/login` 에서 발생한 보호 API 401 → 페이지 재진입(리로드) 없이 머문다
