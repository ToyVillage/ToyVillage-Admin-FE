...
- 포함: 종 조회, 프로필 카드, 카드 케밥(수정·삭제), 종 삭제 확인 모달, 개체 섹션 헤더(마리수·개체 등록 진입),
  개체 표(이름·성별·출생연도), 개체명 검색, 정렬(최신순/오래된순), 페이지네이션, 행 클릭 이동, 행 케밥(수정·삭제), 개체 삭제 확인 모달,
...
- 행 순서는 **기본 최신순**(최근 등록 = 개체 id 큰 순, 종 1 은 두리 → 미미 → 동식이)이다. 검색바 우측 정렬 버튼(`DataTable.sort`) →
  `최신순` / `오래된순`(id 작은 순) 메뉴에서 고르면 그 순서로 바뀌고 1페이지로 리셋된다 (2026-09-15 Figma·저장소 근거 판단: 같은 `filter.svg` 정렬 메뉴 — `NoticeListPage` 선례).
...
- `DataTable`(`src/shared/ui`) — 기존 재사용. `appearance`: `offsetTop: 24`, `headerBackground: 'tableHeaderStrong'`,
  ...
  - `sort`: `DataTableSort { value: 'newest' | 'oldest', onChange, ariaLabel: '개체 정렬' }`(기본 옵션 `최신순` / `오래된순`).
...
- 검색 입력의 접근 가능한 이름은 `개체 검색`, 정렬 버튼은 `개체 정렬` 이다(메뉴 항목 `menuitemradio`, `DataTable` 기존 동작).
...
- S32: 키보드만으로 케밥 열기·항목 실행·행 진입·검색·정렬·페이지 이동을 수행할 수 있다.
...
- 개체 표 정렬은 `DataTable.sort`(최신순/오래된순), 기본 최신순(최근 등록 = id 큰 순)이다 (2026-09-15 Figma·저장소 근거 판단: 같은 `filter.svg` 정렬 메뉴 — `NoticeListPage`·`NoticeReservationsPage` 선례).
