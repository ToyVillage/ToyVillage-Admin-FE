...
- 사진 칩은 저장된 대표 사진의 `photo.fileName` 과 다운로드 컨트롤을 보여준다(제거 버튼 없음).
...
- 사진 칩의 다운로드 버튼 이름은 `${파일명} 다운로드` 다(`AttachmentField` 규칙 승계, 제거 버튼 없음). 사진 거부 문구는 `role="alert"` 다.
...
- S4: 드롭존으로 이미지 한 장 등록 → 사진 칩에 파일명과 다운로드 컨트롤이 보이고 삭제 버튼은 없다.
...
- S24: 수정에서 성별을 바꾼 뒤 사이드바 `개체관리 바로가기` 클릭 → 이탈 확인 모달이 뜬다.
...
- 사진은 이미지(`image/*`) 한 장, 50MB 이하, 새 파일은 교체, 여러 장 동시 드롭은 거부한다. 칩은 Figma(`127:9358`) 순서(유형 아이콘 → 다운로드 → 파일명)이고 제거 버튼이 없다. 거부 문구 3종은 드롭존 아래 인라인 `role="alert"` 다 (2026-09-15 Figma·저장소 근거 판단: Figma `84:8781`·`84:8848` chip, `AttachmentField` 오류 위치).
