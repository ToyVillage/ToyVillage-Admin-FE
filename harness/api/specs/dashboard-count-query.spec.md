---
feature: dashboard-count-query
api_id: DASHBOARD_COUNT_QUERY
target_page: src/pages/dashboard/DashboardPage.tsx
notion_page: https://app.notion.com/p/c0f7a4d61474827ab9f201e2081498d7
...
대시보드 상단 KPI 카드 4개에 이번 주 건수(`GET /dashboard/count`)를 표시한다.
...
- 진입 시 `GET /dashboard/count`를 Bearer 토큰으로 1회 호출한다.
- `먹이 급여 기록`=`feedLogCount`, `개체 관리`=`animalCount`(이번 주 신규 개체), `업무보고`=`workReportCount`, `작성된 일지`=`workLogCount`를 그대로 표시한다.
