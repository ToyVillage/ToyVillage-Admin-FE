...
- `/tasks/:id`(업무 상세)의 `업무 보고 상세조회` 버튼 → `/task-reports` 로 이동한다. 버튼의 `disabled` 를 해제한다.
...
- **TODO-2 (진입 경로):** Figma 사이드바(`1541:1423`~)에 업무보고 항목이 없다. 이번 범위에서는 사이드바를 바꾸지 않고
  업무 상세의 `업무 보고 상세조회` 버튼만 활성화한다.
