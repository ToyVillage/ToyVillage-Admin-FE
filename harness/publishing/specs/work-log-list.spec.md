...
- 세로 배치는 제목 블록 → 탭바 → (`작성된 일지` 탭에서만) `조회날짜` 필터 → 표 순서다.
  탭바는 두 탭에서 같은 자리에 있어 탭을 오가도 움직이지 않는다.
- 화면 진입 → 제목 `업무일지관리`, 부제 `토이빌리지의 업무일지관리`, 우상단 `양식 생성하기` 버튼,
  `작성된 일지` 탭 활성, 그 아래 `조회날짜` 필터, 1페이지 목록이 보인다.
...
- 탭 클릭 → 해당 탭의 목록으로 바뀌고 1페이지로 리셋되며, URL의 `tab` 파라미터가 갱신된다.
  조회날짜 선택값은 탭을 바꿔도 유지된다(양식 관리 탭에서는 필터가 보이지 않을 뿐 값은 남는다).
...
- 열린 드롭다운은 트리거와 펼침 패널이 같은 테두리(`colors.selectOpenBorder`)로 이어져 하나의 팝오버로 읽힌다.
  패널에 테두리가 없으면 아래 흰 표와 색이 겹쳐 경계가 사라진다.
