...
- 사이드바에 `업무 관리 바로가기` 항목을 추가한다. 클릭 → `/tasks` 로 이동하고 사이드바가 닫힌다.
...
- 업무 삭제 실패 조건이 전달된 경우 → 우상단에 `데이터 삭제에 실패했습니다` 토스트를 표시한다.
  단 삭제 실패는 `task-edit` 명세상 수정 화면에 머문 채 알리므로, 이번 슬라이스에는 이 조건을 만드는
  진입 경로가 없다. 코드는 경로를 지원하되 승인 시나리오에서는 제외했다.
...
- 사이드바에 `업무 관리 바로가기` 항목을 추가한다(Figma 사이드바 `1541:1412`에 반영됨).
...
- 완료기한 위험색이 mock 4건 모두에 적용된다. 규칙은 "오늘 이전"이 맞고, Figma mock 날짜가 과거일 뿐이다.
- 테이블 헤더는 `DataTable` 기본값(좌측 정렬, `colors.tableHeader`)을 쓴다. Figma의 가운데 정렬과
  `#DDDDE3` 을 맞추려면 `DataTable` API 변경이 필요해 재사용을 우선했다.
