...
paths: src/pages/notices/notice/EditNoticePage.tsx, src/features/create-notice, src/entities/notice
---
...
- 수정 화면 기준: yot Figma `1:6711` (섹션 `공지사항 · 수정` `309:12760`)
- 2026-09-17 변경: 라우트를 `/notices/list/:id/edit`로 옮기고, 삭제는 목록 케밥으로 이동해 이 화면의 `삭제하기`를 제거한다.
...
- 라우트: `src/app/App.tsx`의 `/notices/list/:id/edit`

Figma 구조 API는 Starter 플랜 호출 한도로 사용할 수 없었고, 같은 노드의 원본 크기 캡처는 정상 확보했다. 아래 시각 수치는 캡처와 기존 생성 폼의 일치하는 치수를 근거로 한다.

## 목적

운영 관리자가 기존 공지의 제목, 분류, 내용과 첨부파일을 수정한다. 저장 실패 또는 실수로 인한 이탈에도 입력과 기존 데이터를 잃지 않아야 한다.

## 범위

- 포함: 기존 공지 조회, 제목·분류·내용 편집, 첨부 확인·다운로드·추가·제거, 저장, 이탈 보호
- 제외(이동): 삭제 — 목록 케밥(`notice-list.spec.md`)
- 제외: 게시 예약, 공개 범위, 리치 텍스트, 첨부 파일 서버 업로드 본문, 수정 이력

## 라우트와 진입

- 공지 목록 행 케밥의 `수정` → `/notices/list/:id/edit`로 이동한다.
- `/notices/list/:id/edit` → 해당 ID의 공지 수정 화면을 표시한다.
- 저장 성공 → `/notices/list`로 이동한다.
...
6. 우측 하단 액션: `저장하기` black solid, 24px SemiBold 하나만 둔다.

라벨: 제목 카드 `제목`, 내용 카드 `상세 업무 내용 *`(Figma 문구 그대로. #95의 문구 확인 항목은 디자이너 답변 전까지 Figma 기준 유지).
...
- 분류 pill 선택·팀 제거 동작은 생성 화면과 동일하다. yot 수정 화면에는 `+ 팀 추가`가 있으므로 생성 화면과 같이 표시한다.
...
- 기본 키보드 순서는 제목 → 분류 → 내용 → 첨부 control → 파일 업로드 → 저장하기다.
...
- S1: 목록 첫 행 케밥 → `수정` → 해당 `/notices/list/:id/edit` URL로 이동한다.
