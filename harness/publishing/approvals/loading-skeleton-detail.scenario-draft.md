...
### S22: 상세 조회 중에도 정적 UI는 실제 UI로 보인다
- Given: 공지사항 상세 조회 응답을 지연시켰다
- When: `/notices/list/:id` 에 진입
- Then: `뒤로가기` 링크와 카드 라벨 `분류`·`날짜`·`첨부자료`가 글자로 보인다

### S23: 수정 조회 중에도 폼 라벨은 실제 UI로 보인다
- Given: 공지사항 수정 조회 응답을 지연시켰다
- When: `/notices/list/:id/edit` 에 진입
- Then: 폼 라벨이 막대가 아니라 글자로 보인다

### S24: 자료실 상세 조회 중 스켈레톤
- Given: 자료실 상세 조회 응답을 지연시켰다
- When: `/notices/resources/:id` 에 진입
- Then: `불러오는 중` status(`aria-busy=true`) 스켈레톤이 보이고, `뒤로가기`·`분류`·`날짜`·`첨부자료` 는 글자로 보인다
